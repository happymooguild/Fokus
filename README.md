...
Fokus isn't on the Chrome Web Store yet. To run it from source:

1. Open `chrome://extensions`
2. Turn on **Developer mode** (top right)
3. Click **Load unpacked** and pick this folder

After editing any file, hit the reload icon on the Fokus card, then reload
your YouTube tabs.
...
`src/content/content.js` keeps `<html data-fokus="...">` in sync with
...
- **Autoplay**, which is a one-shot rather than a rule. Fokus switches
  autoplay off once per page and then leaves the control alone. Switch it back
  on in the player and the Fokus toggle follows you off, so the popup never
...
src/
  ...
  content/
    fokus.css              all the hiding rules
...
`src/content/fokus.css`. The popup builds itself from the former.
...
If Fokus is useful to you, there's a **Support me** link at the bottom of the
